...
# I'm not sure if this is really necessary any more...
#
case $0 in
*sh)    # sourced from /bin/sh ...
    unset JED_ROOT JED_LIBRARY
    ;;
esac
...
then
    ...
    then
        ...
        [ -d "$slanglib" ] && JED_LIBRARY="$slanglib,${JED_LIBRARY}"

        for tmp in /usr /usr/local $HOME/usr $HOME/usr/local
        do
            tmp="$tmp/slang/lib"
            if [ -d "$tmp" -a "$tmp" != "$slanglib" ]
            then
    ...
    case $0 in
        ...
        *)
            bg=;
            cmd=$JED_ROOT/bin/$(basename $0)
            case $cmd in
            */xjed.bg | */Xjed)     # execute in background mode
                cmd=$JED_ROOT/bin/xjed
                bg='&'
                ;;
            esac

            [ -x "$cmd" ] || { echo "$cmd not found"; exit 1; }
